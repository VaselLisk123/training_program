...
def create_product(row,value,location_id):
    try:
        product = value.get("create product")(row,value)
    except CantCreateProduct:
        #Запись в лог
        return None 
    variant = value.get("create product variant")(row,product,value)
...
def upload_products():
    ...
    for header,value in sheet_fields.items():
        ...
        for i,row in excel_info.iterrows():
            check_sku = (str(row[value.get("CentreSoft Code")]))
            ...
            if current_variant:
                ...
                create_product(row,value,default_location_id)
